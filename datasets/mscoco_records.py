...
def write_to(writer, cocoid, img_paths, objects, nouns, n_nouns, cnn):
    """
    ...
    nouns_vector = np.zeros(n_nouns, dtype="int")
    nouns_vector[nouns] = 1
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'nouns':  _bytes_feature(nouns_vector.tostring()),
...
def write_tfrecords(writer, ids, coco_obj, img_paths, n_nouns, cnn):
    ...
    for c, i in enumerate(ids):
        ...
        int_nouns = list(set([nouns_dict.get(x, 0) for x in nouns]))
        # int_caps, lengths = encode_caps(caps)            # captions matrix, L
        write_to(writer, i, img_paths, int_objs, int_nouns, n_nouns, cnn)
...
if __name__ == "__main__":
    ...
    print "Writing records for training set", len(ids)
    writer = tf.python_io.TFRecordWriter(out_dir + "/train_mscoco_vgg.tfrecords")
    write_tfrecords(writer, ids, coco_obj, train_imgs, len(nouns_dict), cnn)
    writer.close()
    print "Done"

    print "Writing records for validation set", len(ids)
    ...
    ids = coco_obj.getImgIds()       # ids of val set

    writer = tf.python_io.TFRecordWriter(out_dir + "/val_mscoco_vgg.tfrecords")
    write_tfrecords(writer, ids, coco_obj, val_imgs, len(nouns_dict), cnn)
    writer.close()
